<?php
//Imagen pregunta original
    $src_pregunta = '';

    if ( strlen($row->archivo_imagen) )
    {
        $src_pregunta = URL_UPLOADS . 'preguntas/' . $row->archivo_imagen;
    }
...
//Imagen versión
    ...
    if ( strlen($row_version->archivo_imagen) )
    {
        $src = URL_UPLOADS . 'preguntas/' . $row_version->archivo_imagen;
        $has_image = TRUE;
        $cl_form = 'd-none';
        $cl_img = '';
    }
...
//-----------------------------------------------------------------------------
    ...
    function send_form()
    {
        ...
        $.ajax({        
            ...
            contentType: false,
            cache: false,
            url: app_url + 'preguntas/set_image/' + pregunta_id,
            data: form_data,
            beforeSend: function(){
...
<div id="add_file" class="row">
    <div class="col-md-5 offset-md-2">
        <img src="<?php echo $src_pregunta ?>" alt="Imagen archivo pregunta original" style="width: 100%;">
    </div>
    ...
    <div class="col-md-5">
        ...
        <form accept-charset="utf-8" method="POST" id="file_form" class="<?php echo $cl_form ?>">
            <div class="alert alert-info" id="status_text">
                La pregunta alterna no tiene imagen asociada
            </div>
