...
<ul class="nav nav-tabs sep2">
    ...
    <?php if ( $this->session->userdata('srol') == 'interno' ) { ?>
        <li role="presentation" class="<?= $clases['asignaciones'] ?>">
            <?= anchor("cuestionarios/asignaciones", '<i class="fa fa-users"></i> Asignaciones', 'title="Explorar asignación de cuestionarios"') ?>
        </li>
    <?php } ?>
