<?php
    ...
    //Archivos de audio
        $carpeta = base_url() . RUTA_UPLOADS .'audios/';
        $ruta_audio = $carpeta . $row->archivo_audio;
        $archivo_audio = $this->Pcrn->si_vacia($row->archivo_audio, 'ND');
    
    //Colores
        $colores = $this->App_model->arr_color_area();
    
    //Atributos
        $atributos_style['background'] = $colores[$row->area_id];
    
?>

<div class="div2">
    <span class="etiqueta" style="<?= $this->App_model->style($atributos_style) ?>"><?= $this->App_model->nombre_item($row->area_id) ?></span>
    <span class="etiqueta nivel w1"><?= $row->nivel ?></span>
    <span class="suave"> | Código: </span>
    <span class="resaltar"><?= $row->cod_tema ?></span>
</div>
